...
class _FaceRegistrationScreenState extends State<FaceRegistrationScreen> with TickerProviderStateMixin {
  ...
  _setup() async {
    ...
    if (frontCamera == null) {
      if (mounted) AppDialog.showError(context, "Kamera tidak ditemukan");
      return;
    }
  ...
  void _startLiveDetection() {
    _ctrl.startImageStream((image) async {
      if (_isProcessing || _cooldown || _actions[_currentStepIndex] == FaceAction.done || _regStep != 0) return;
      _isProcessing = true;
      try {
        final faces = await _faceService.detector.processImage(_faceService.getInputImageFromCameraImage(image, _ctrl.description));
        if (faces.isNotEmpty) {
  ...
  void _checkGesture(Face face) async {
    ...
    debugPrint("Reg Face Angles -> Y: ${face.headEulerAngleY?.toStringAsFixed(2)}, X: ${face.headEulerAngleX?.toStringAsFixed(2)}");
    ...
    if (success) {
      ...
      setState(() => _hint = "Posisikan: ${_getHint(currentAction)}");
    }
  }

  Future<void> _takeSilentSnapshot() async {
    try {
      final f = await _ctrl.takePicture();
      _capturedImages.add(f.path);
    } catch (e) {
      debugPrint("Gagal ambil snapshot: $e");
    }
  }
  ...
  void _saveFace() async {
    if (_capturedImages.isEmpty || _isBusy) return;
    setState(() => _isBusy = true);
    try {
      List<String> base64Images = [];
      for (var path in _capturedImages) {
        final b64 = await _faceService.getFaceBase64(path);
      ...
      AppDialog.showError(context, "Gagal terhubung ke server: $e");
    }
  }
...
class CircularTicksPainter extends CustomPainter {
  final double progress;
  CircularTicksPainter(this.progress);
  @override
